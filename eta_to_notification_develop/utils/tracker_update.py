...
class TrackerUpdate():
    ...
    @staticmethod
    async def is_arrived(trace: TravelData, tracker_update: TrackerUpdate) -> bool:

        tracker_coordinates = (tracker_update.lat, tracker_update.long)
        # eta_unix_time = time.mktime()

        for stop in trace.stops:
            stop_coordinates = (stop.arrivalLatitude, stop.arrivalLongitude)
            # eta_unix_time = time.mktime(stop.arrivalTime.timetuple())
            dis = distance.distance(tracker_coordinates, stop_coordinates).m
            if dis < 10:
